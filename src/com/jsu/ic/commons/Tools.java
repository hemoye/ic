...
public class Tools {
	...
	public static long ipLongValue(String ip) {
		...
		try {
			String[] ips = ip.split("\\.");
			return Long.parseLong(ips[0]) * 256 * 256 * 256 + Long.parseLong(ips[1]) * 256 * 256 + Long.parseLong(ips[2]) * 256
	...
	public static String URLEncode(String key, String code) throws Exception {
		return java.net.URLEncoder.encode(key, code).replaceAll("%5C", "/").replaceAll("%2F", "/");
	}
	 ...
	 * @return
	 */
	public static StringBuffer replace(StringBuffer oldStr, String fromStr, String toStr) {
	 ...
	 * @throws Exception
	 */
	public static String subString(String title, int len, String addString) throws Exception {
		try {
	...
	public static String formatMoney(Number m) {
		...
		if (tempD.longValue() < 10) {
			...
			int len = v.length();
			String m1 = v.substring(0, len - 3) + "." + v.substring(len - 3, len - 1);
